...
from CPS.config import config, str_to_int_list
    ...
    for input_val, expected in test_cases:
        ...
        assert result == expected, f"转换失败: {result} != {expected}"
    ...
    # 检查是否为列表
    assert isinstance(args.k_list, list), f"k_list不是列表: {type(args.k_list)}"
    assert isinstance(args.inr_latent, list), f"inr_latent不是列表: {type(args.inr_latent)}"
    assert isinstance(args.decoder_latent, list), f"decoder_latent不是列表: {type(args.decoder_latent)}"
    ...
    assert args2.k_list == [2, 4, 6, 8], f"k_list解析错误: {args2.k_list}"
    assert args2.inr_latent == [128, 256, 512], f"inr_latent解析错误: {args2.inr_latent}"
    assert args2.decoder_latent == [512, 256], f"decoder_latent解析错误: {args2.decoder_latent}"
